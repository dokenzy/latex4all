...
Mac OS에서는 \XeTeX을 위한 설정을 한 번 해주어야 한다. \footnote{윈도우즈는 \texttt{fc-cache}명령을 실행해야 한다. ko.TeX Live 2013을 설치하면 자동으로 해준다.}
...
\end{enumerate}
같은 방식으로 truetype 폰트들도 추가할 수 있다.\footnote{/usr/local/texlive/2013/texmf-dist/fonts/truetype/public} 이제 Mac에서 \XeTeX을 사용하기 위한 설정을 모두 마쳤다.
...
\TeX은 강력한 조판시스템이다. 강력한 만큼 배울 게 많다. 앞의 예제코드 역시 매우 간단한 코드이지만 그 속에는 많은 사연이 들어있다. 본격적으로 시작하기 전에 예제코드를 꼼꼼히 살펴보겠다.

\subsection{클래스(document class)}
예제코드의 \verb|\documentclass[oneside]{article}|부분이다.
클래스는 문서의 성격을 결정하는 가장 중요한 부분으로 tex문서를 작성할 때 반드시 가장 먼저 한 번만 쓰인다. 템플릿처럼 문서 전체의 모양을 결정할 뿐만 아니라, 많은 기능과 옵션을 제공한다. 예컨데 예제코드의 \verb|\section{시작하기}| 명령은 article 클래스가 제공하기 때문에 사용할 수 있다. 하지만 \verb|\chapter{시작하기}|명령을 사용하면 오류가 발생한다. article 클래스에는 없는 명령이기 때문이다. 또다른 표준클래스인 report 클래스는 \verb|\chapter{}|명령을 제공하기 때문에 사용할 수 있다.

oneside 옵션은 article 클래스가 제공하는 옵션이다. 이 외에 a4paper, 11pt, landscape 등의 옵션들이 있다. 어떤 옵션을 쓸 수 있을지는 전적으로 클래스에 달려있다. 만약 클래스에서 제공하지 않는 기능들을 사용하고 싶다면, 관련 패키지를 사용하거나 직접 정의(\ref{sec:newcommand}\chaptername~\titleref{sec:newcommand})해야 한다.
%
\subsection{명령(command)}\label{subsec:command}
명령을 사용하면 이 명령 뒤에 있는 내용 전체에 영향을 미쳐서 원하지 않은 결과를 얻을 수 있다. 따라서 일정 구역에만 명령을 적용하기 위해서는 다음과 같은 방법을 사용한다.
...
옵션이 있는 명령도 있고 없는 명령도 있다. \verb|\section[시작]{\LaTeX 시작하기}|는 옵션이 있는 명령이고, \verb|\LaTeX|은 없는 명령이다. 또 옵션이나 인자가 2개 이상인 명령도 있다. 만약 텍 문서를 컴파일 하는 중에 \texttt{! Undefined control sequence.} 오류가 발생한다면, 명령에 오타가 있거나 정의되지 않은 명령(클래스나 사용하는 패키지에 없는 명령)을 쓴 것이다. 이럴 때는 해당 명령을 제공하는 클래스나 패키지 매뉴얼을 읽어봐야 한다. 매뉴얼에 대한 설명은 \ref{sec:texdoc}\sectionname~\titleref{sec:texdoc}을 참조한다.

\subsection{환경(environment)}\label{subsec:environment}
환경은 예제의 \verb|\begin{center}|\ldots\verb|\end{center}|와 같은 형식을 갖는다. 옵션의 위치는 환경에 따라 조금씩 다를 수 있다.

\verb|\begin{document}|\ldots\verb|\end{document}|도 환경이다. document 환경은 \LaTeX 문서라면 반드시 하나만 존재해야 하는 중요한 환경이다. 마치 HTML의 \verb|<body>|태그와 같다. 환경 역시 정확한 사용방법은 매뉴얼을 보는 것이다.
...
\setmainhangulfont{HCR Batang LVT}
\end{pyglist}
가 위치한 곳이 바로 프리앰블이다. HTML의 \verb|<head>|태그와 비슷한 기능을 하지만 프리앰블을 명시하는 별도의 명령은 없다. 패키지를 불러오거나(\verb|\usepackage{}|), 사용자가 명령이나 환경을 정의할 때에는 프리앰블에 해야 한다.
...
\% 문자에 대해서는 여기서 설명하는 것이 좋겠다. \%는 주석(comment)을 나타내는 특수문자이다. 따라서 \LaTeX이 컴파일 할 때 무시해 버린다. HTML의 `\verb|<!-- -->|', Python의 `\verb|#|'\과 같다.

\subsection{따옴표}
작은따옴표나 큰따옴표를 입력해야 하는 경우가 많다. 이 때 여는 따옴표와 닫는 따옴표의 모양이 조금 다르다. \LaTeX에서는 다음과 같이 입력하면 이들 따옴표를 이쁜 따옴표\footnote{curly quotation mark}로 만들어 준다.\verbfootnote{프리앰블에서 \verb|\defaultfontfeatures{Ligatures=TeX}| 명령을 선언해야한다. 더 궁금한 사람은 KTUG QnA 게시판에 두텁님이 쓰신 \href{http://www.ktug.org/xe/index.php?mid=KTUG_QnA_board&page=2&document_srl=176043}{답글}을 읽어보자.}
...
여는 따옴표(\verb|`|)는 키보드에서 숫자 1 왼쪽에 있는 키(틸데(\textasciitilde)와 같은 자리)이고, 닫는 따옴표(\verb|'|)는 키보드에서 엔터키 왼쪽에 있는 키(\verb|"|와 같은 자리)이다.\footnote{이 배열은 QWERTY 키보드를 기준으로 한 설명이다.}
...
\verb|texdoc|명령을 이용하면 설명서를 쉽게 찾아서 읽을 수 있다. 대부분의 클래스나 패키지의 설명서는  \texttt{<texdoc 패키지이름>}과 같은 방식으로 읽을 수 있다. 예를 들어 \koTeX에 대한 설명서를 읽기 위해서는 터미널 또는 명령프롬프트에서 \texttt{<texdoc kotex>} 명령을 실행하면 된다. 그러면 시스템의 PDF기본 뷰어로 kotexdoc.pdf 파일이 열린다.
...
간혹 단락제목이 너무 길면 차례에서 볼 때 불편한 경우가 생길 수 있다. 이 때는 \verb|\chapter[short title]{very long chapter title}|과 같이 옵션을 주면, 차례에는 `short title'이, 본문에는 `very long chapter title'이 들어간다.
...
만약 순서가 중요한 경우라면 itemize 환경 대신 enumerate 환경을 이용하자. \TeX이 알아서 번호를 매겨준다. HTML의 \verb|<ol>|태그와 같다.
\begin{pyglist}
\begin{enumerate}
\item 좋아하는 연예인 이름
\item 좋아하는 편집기
...
중요한 점은 최소 두 번 이상 컴파일해야 차례를 만들 수 있다. 한 번 컴파일하고 나서 PDF문서를 보면 차례에 아무 것도 나오지 않는 것을 쉽게 발견할 것이다. 하지만 foo.tex파일과 같은 폴더에 foo.toc 파일이 만들어졌을 것이다. 왜 두 번 이상 컴파일 해야 하는지, 어떻게 자동으로 만들어지는지 궁금한 사람은 foo.toc 파일을 살펴보기 바란다.
...
\begin{showresult} 
신데렐라\과 백설공주\은 왕자\를 사이에 두고 머리끄댕이\을 잡고 싸웠다더\이라
\end{showresult}
...
예컨데, 이 문서에 쓰인 한글본문폰트는 다음과 같이 설정했다.
\begin{pyglist}
\setmainhangulfont{HCR Batang LVT}
\end{pyglist}
...
이제 남은 것은
\begin{inparaenum}[1)]
\item 스스로 해결하거나, \item 구글에서 검색하거나, \item KTUG에 질문하는 것이다.
\end{inparaenum}

스스로 해결하거나 구글에서 검색하는 것은 직접 해보면 되고, 이 문서에서는 KTUG에 질문을 올리는 요령을 간략히 설명한다.

\begin{enumerate}
\item MWE을 만든다.
\item 그림파일이 있다면 그림파일도 포함한다.
\item KTUG의 질문게시판에 자신의 텍 환경\footnote{운영체제, TeXLive 버전, 사용한 텍 엔진 등}과 오류내용, 원하는 결과 등을 적고
\item 파일을 첨부해서 올린다.
\end{enumerate}
...
\item 한글식 카운터: \jaso{index}, \pjaso{index}, \ojaso{index}, \gana{index}, \ogana{index}, \pgana{index}, \onum{index}, \pnum{index}, \oeng{index}, \peng{index}, \hnum{index},\Hnum{index}, \hroman{index}, \hRoman{index}
\item 자동조사
...
자세한 내용은 kotexguide를 읽어보기 바란다. 여기서는 세로쓰기의 예만 간략히 보이겠다.

\begin{figure}[!hbtp]
\begin{center}
\begin{vertical}{11em}
\hangulfontspec[ Vertical=RotatedGlyphs]{a아시아실록.otf}
...
\item[\href{http://faq.ktug.org/faq/Karnes/2010-09?action=download&value=latex4wp-ko-7.pdf}{워드 프로세서 사용자를 위한 \LaTeX}] 김강수
...
\chapter*{마치며}
근본없는 \LaTeX 매뉴얼은 여기까지다. 힘들어서 맞춤법 수정도 포기했다. tex 소스가 겨우 1050라인에 불과한, A4기준 50쪽도 안되는 문서인데도 불구하고 힘들다. memoir설명서나 lshort-kr같은 문서를 번역하신 분들의 노고가 새삼 더 크게 느껴진다.
